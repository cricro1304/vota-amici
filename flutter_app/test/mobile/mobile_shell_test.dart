// Widget test for [MobileShell] + [MobileHomeTab].
//
// MobileShell is the mobile-only scaffold (bottom nav + status-bar tint +
// tap-outside-to-dismiss-keyboard gesture) that wraps every mobile tab.
// MobileHomeTab is the "Gioca" tab — a thin passthrough to the shared
// [HomeScreen].
//
// Together they form the mobile entry point. That sounds trivial, but
// the shell's GestureDetector + AnnotatedRegion are exactly the kind of
// wrappers that can silently break rendering if the hit test gets eaten
// or the overlay wraps the wrong subtree — so this test acts as a smoke
// check before we take the app into Xcode.
//
// Scope (deliberately minimal):
//   1. The home-screen CTAs ("Gioca Ora" / "Unisciti") still render when
//      MobileHomeTab is hosted inside MobileShell. If this fails, the
//      shell broke the shared flow for phone users.
//   2. An [AnnotatedRegion] with [SystemUiOverlayStyle.dark] is present in
//      the tree. This is what controls the iOS status-bar tint.
//   3. A [GestureDetector] with [HitTestBehavior.opaque] is present. This
//      is what lets taps on empty space dismiss the soft keyboard.
//
// We override [isOnlineProvider] rather than letting the real
// `connectivity_plus` plugin run — `MethodChannel` calls have no backing
// implementation under `flutter_test` and would throw a MissingPluginException
// when [GameLayout] subscribes to the stream during build.

import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:go_router/go_router.dart';

import 'package:vota_amici/mobile/mobile_shell.dart';
import 'package:vota_amici/mobile/screens/mobile_home_tab.dart';
import 'package:vota_amici/screens/home_screen.dart';
import 'package:vota_amici/state/connectivity_provider.dart';

void main() {
  /// Pumps [MobileShell] hosting [MobileHomeTab] inside the minimum
  /// scaffolding it needs to render: a [GoRouter] (MobileShell reads the
  /// current path via `GoRouterState.of(context)`), a [MaterialApp.router]
  /// for directionality / theme, and a [ProviderScope] with
  /// [isOnlineProvider] stubbed so `GameLayout` doesn't hit the
  /// `connectivity_plus` plugin.
  Future<void> pumpMobileShell(WidgetTester tester) async {
    final router = GoRouter(
      initialLocation: '/mobile',
      routes: [
        ShellRoute(
          builder: (_, __, child) => MobileShell(child: child),
          routes: [
            GoRoute(
              path: '/mobile',
              builder: (_, __) => const MobileHomeTab(),
            ),
          ],
        ),
      ],
    );

    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          // Keep the offline banner hidden; the real provider would call
          // `Connectivity()` which has no platform channel backing in tests.
          isOnlineProvider.overrideWith((ref) => Stream.value(true)),
        ],
        child: MaterialApp.router(routerConfig: router),
      ),
    );
    // One `pump` kicks off the PopIn animation controllers; we don't
    // pumpAndSettle because Google Fonts and network emoji images will
    // never "settle" in an offline test environment. The first frame is
    // enough to assert the widget tree is well-formed.
    await tester.pump();
  }

  group('MobileShell + MobileHomeTab', () {
    testWidgets(
        'Given the mobile shell hosting the home tab, '
        'When it renders, '
        'Then the shared home CTAs are visible', (tester) async {
      await pumpMobileShell(tester);

      // EmojiText renders as Text.rich, so the button labels live inside
      // RichText widgets. `findRichText: true` tells the finder to search
      // TextSpan content in addition to plain Text widgets.
      expect(
        find.textContaining('Gioca Ora', findRichText: true),
        findsOneWidget,
        reason: 'Primary CTA should render inside the mobile shell',
      );
      expect(
        find.textContaining('Unisciti', findRichText: true),
        findsOneWidget,
        reason: 'Secondary "join room" CTA should render too',
      );
    });

    testWidgets(
        'Given the mobile shell, '
        'When it renders, '
        'Then a dark status-bar AnnotatedRegion wraps the content',
        (tester) async {
      await pumpMobileShell(tester);

      // There can be more than one AnnotatedRegion<SystemUiOverlayStyle>
      // in the tree (MaterialApp injects its own based on theme), so we
      // specifically look for one descendant of MobileShell whose value
      // is SystemUiOverlayStyle.dark.
      final annotated = find.descendant(
        of: find.byType(MobileShell),
        matching: find.byWidgetPredicate(
          (w) =>
              w is AnnotatedRegion<SystemUiOverlayStyle> &&
              w.value == SystemUiOverlayStyle.dark,
          description:
              'AnnotatedRegion<SystemUiOverlayStyle> with dark status-bar style',
        ),
      );
      expect(
        annotated,
        findsOneWidget,
        reason:
            'MobileShell should force dark status-bar icons so the time / '
            'battery stay legible against the light scaffold background.',
      );
    });

    testWidgets(
        'Given the mobile shell, '
        'When it renders, '
        'Then an opaque GestureDetector catches taps to dismiss the keyboard',
        (tester) async {
      await pumpMobileShell(tester);

      // Multiple GestureDetectors can appear in the tree — InkWell under
      // each ElevatedButton wires one up with HitTestBehavior.opaque too —
      // so we specifically look for a GestureDetector inside MobileShell
      // that wraps a Scaffold (that's the shape MobileShell builds) and
      // has an onTap. That combination is unique to the shell's
      // tap-outside-to-dismiss-keyboard handler.
      final detector = find.descendant(
        of: find.byType(MobileShell),
        matching: find.byWidgetPredicate(
          (w) =>
              w is GestureDetector &&
              w.behavior == HitTestBehavior.opaque &&
              w.onTap != null &&
              w.child is Scaffold,
          description:
              'GestureDetector with HitTestBehavior.opaque wrapping a Scaffold',
        ),
      );
      expect(
        detector,
        findsOneWidget,
        reason:
            'Without the opaque detector, tapping outside a focused TextField '
            'wouldn\'t dismiss the soft keyboard — the primary CTA would stay '
            'covered and the flow would feel broken on iOS.',
      );
    });

    testWidgets(
        'Given the mobile shell hosting the home tab, '
        'When it renders, '
        'Then HomeScreen is reachable through MobileHomeTab', (tester) async {
      await pumpMobileShell(tester);

      // MobileHomeTab is a passthrough to the shared HomeScreen. If this
      // fails, the tab broke its contract with the shared flow.
      expect(
        find.descendant(
          of: find.byType(MobileHomeTab),
          matching: find.byType(HomeScreen),
        ),
        findsOneWidget,
        reason:
            'MobileHomeTab should render HomeScreen unchanged so the shared '
            'create/join flow keeps working on mobile.',
      );
    });
  });
}
